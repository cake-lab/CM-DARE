...
from tensor2tensor.utils import saving_hook ## Master switch
# from tensor2tensor.utils import get_step_hook ## Extra utility
...
def create_session_config(log_device_placement=False,
  ...
  if len(tf_config) != 0:
    if tf_config['task']['type'] == 'master':
    ...
    elif tf_config['task']['type'] == 'chief':
      ...
      device_filters = ['/job:ps', '/job:worker', '/job:master', '/job:chief']
  ...
  if tf_config:
    out_file2 = open("/home/ozymandias/trainingStart.log","w")
    out_file2.write(str(datetime.utcnow()))
    # then = time.time()
    # out_file2.write(str(then))
    out_file2.write("\n")
    # logging.basicConfig(filename='/home/ozymandias/serverStart.log', filemode='w', level=logging.DEBUG)
    # logging.debug('Sending start signal to server: ' + str(time.time()))
    ### Start signal
    # xml_server.serverStarted(tf_config['task']['type'], tf_config['task']['index'], socket.gethostname())
    # out_file2.write(str(datetime.utcnow()))
    # now = time.time()
    # out_file2.write(str(now))
    # out_file2.write("\n")
    # out_file2.write(str(now - then))
    out_file2.close()
    # logging.debug('Finished sending start signal to server: ' + str(time.time()))
    out_file = open("/tmp/job_config.txt","w")
    out_file.write(str(tf_config['task']['type']))
    out_file.write("\n")
    out_file.write(str(tf_config['task']['index']))
    out_file.close()
...
def create_hooks(use_tfdbg=False,
  ...
  if use_dbgprofile:
    ...
    defaults = dict(save_steps=50, show_dataflow=True, show_memory=True)
...
def create_experiment(
  ...
  tf_config = json.loads(os.environ.get('TF_CONFIG', '{}'))
  if tf_config:
    train_hooks.append(saving_hook.custom_saver_hook(run_config.model_dir, save_steps=run_config.save_checkpoints_steps)) ## Master switch
    # train_hooks.append(get_step_hook.custom_step_hook())
